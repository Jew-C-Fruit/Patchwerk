...
async def main():
    async with aiohttp.ClientSession() as s:
        async with s.ws_connect("http://127.0.0.1:8765/ws") as ws:
            ...
            base = statistics.mean(silence) if silence else 0
            heard = statistics.mean(loud) if loud else 0
            print(f"input level: silence={base:.5f}  playing={heard:.5f}  "
                  f"ratio={heard / max(base, 1e-6):.1f}x")
            if heard > max(base * 3, 0.001):
