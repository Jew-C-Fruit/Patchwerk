...
@module(
    ...
    params={
        ...
        "window": param(0.02, 0.2, 0.04, curve="exp"),  # grain size — ALSO the latency
        "smear": param(0.0, 0.02, 0.002),   # time dispersion: 0 = robotic, up = airy
    },
)
@synthdef()
def pitchshift(in_bus=0, out=0, semitones=0, mix=1.0, window=0.04, smear=0.002):
    dry = In.ar(bus=in_bus, channel_count=2)
    ratio = Lag.kr(source=semitones, lag_time=0.05).semitones_to_ratio()
    wet = PitchShift.ar(source=dry, window_size=window, pitch_ratio=ratio,
